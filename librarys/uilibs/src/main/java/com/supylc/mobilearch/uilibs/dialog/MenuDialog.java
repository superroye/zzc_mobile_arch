...
public class MenuDialog extends BaseDialog implements View.OnClickListener {
    ...
    public MenuDialog setTitle(String title) {
        if (!TextUtils.isEmpty(title)) {
            if (mTitle == null) {
                mTitle = new Menu(title, ContextCompat.getColor(Utils.getApp(), com.supylc.mobilearch.design.R.color.textSecondary), 16, false);
            } else {
                mTitle.text = title;
            }
        }
        ...
        public View getView(Context context) {
            if (view == null) {
                ...
                LinearLayout.LayoutParams lp = new LinearLayout.LayoutParams(
                        LinearLayout.LayoutParams.MATCH_PARENT, T.UI.dp2px(50));
                lp.bottomMargin = 1;
